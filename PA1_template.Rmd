...
```                        
The Mean number of steps is **`r MeanNumberOfSteps`**

```{r print median nr of steps per day}
    MedianNumberOfSteps<-(median(StepsDay$steps))
```

The Median number of steps is **`r MedianNumberOfSteps`**
...
```
The maximum number of steps (i.e. `r MaxstepsAmount` ) is on interval  **`r MaxstepsInterval`**

## Imputing missing values

Total number of missing values in the dataset (i.e. the total number of rows with NAs)
```{r nr of missing values}
    MissingNum<-length(which(is.na(ActMonDat$steps)))
   
```
 
In total there are **`r MissingNum`** observations with missing values.
...
```{r print mean nr of steps per day 2}
    MeanNumberOfSteps2<-mean(StepsDay2$steps)
```
The Mean number of steps is **`r MeanNumberOfSteps2`**

Median Number of steps
```{r print median nr of steps per day2}
    MedianNumberOfSteps2<-median(StepsDay2$steps)
```

The Median number of steps is **`r MedianNumberOfSteps2`**
 
The impact of imputing missing data on the estimates of the total daily number of steps is that the mean remains the same while the median changes from **`r MedianNumberOfSteps`** to **`r MedianNumberOfSteps2`**  and becomes equal to the Mean.

## Are there differences in activity patterns between weekdays and weekends?

```{r weekdays vs weekends,fig.width=12, fig.height=10}
...
The comparison of both graps shows that there are differences in Weekday activities and Weenend activities.
